...
function SplitStrings:apply(ast, pipeline)
	...
	visitAst(ast, function(node, data) 
		...
		if(node.kind == Ast.AstKind.StringExpression) then
			...
			-- Split String into Parts based on Strategy
			if self.Strategy == "pattern" then
				-- Pattern mode: use consistent split length for predictability
				local avgLen = math.floor((self.MinLength + self.MaxLength) / 2)
				while i <= string.len(str) do
					table.insert(chunks, string.sub(str, i, i + avgLen - 1));
					i = i + avgLen;
				end
			else  -- random (default)
				-- Random mode: vary length between MinLength and MaxLength
				while i <= string.len(str) do
