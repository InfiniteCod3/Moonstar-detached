...
local RandomLiterals = require("moonstar.randomLiterals")
...
function ProifyLocals:apply(ast, pipeline)
    -- Determine probability based on Mode setting
    local probability = self.Probability or 0.5
    if self.Mode == "light" then
        probability = probability * 0.5  -- Less aggressive
    elseif self.Mode == "aggressive" then
        probability = probability * 1.5  -- More aggressive (capped at 1.0)
        if probability > 1.0 then probability = 1.0 end
    end
    -- standard mode uses base probability
    ...
    local function getLocalMetatableInfo(scope, id)
        ...
        -- Apply based on probability (Mode-adjusted)
        if math.random() > probability then
            -- Don't proxify this variable
